...
internal object Huffman {
    ...
    private fun decodeHuffman(message: ByteArray, length: Int): String {
        try {
            ...
            while (true) {
                ...
                var charID: Int
                if (HUFFMAN_DECRYPT_KEYS[keyIndex].also { charID = it } < 0) {
                    sb.append(Char(charID.inv().toByte().toUShort()))
                    if (length <= ++charsDecoded) {
                        break
                    }
                    keyIndex = 0
                }
                if (character.toInt() and 0x40 != 0) {
                    keyIndex = HUFFMAN_DECRYPT_KEYS[keyIndex]
                } else {
                    keyIndex++
                }
    ...
    private fun encodeHuffman(text: ByteArray, dest: ByteArray): Int {
        try {
            var key = 0
            var position = 1 shl 3
            for (b in text) {
                val character: Int = b.toInt() and 0xff
                val mask = HUFFMAN_MASKS[character]
                val size = HUFFMAN_BIT_SIZES[character].toInt()
                ...
                bitOffset += 24
                dest[offset] = (key or (mask ushr bitOffset)).also { key = it }.toByte()
                if (byteSize <= offset) continue
                offset++
                bitOffset -= 8
                dest[offset] = (mask ushr bitOffset.also { key = it }).toByte()
                if (byteSize <= offset) continue
                offset++
                bitOffset -= 8
                dest[offset] = (mask ushr bitOffset.also { key = it }).toByte()
                ...
                offset++
                dest[offset] = (mask ushr bitOffset.also { key = it }).toByte()
                if (offset >= byteSize) continue
                bitOffset -= 8
                offset++
                dest[offset] = (mask shl -bitOffset.also { key = it }).toByte()
            }
            return 7 + position shr 3
        } catch (ignored: Throwable) {
            return 0
        }
    }
